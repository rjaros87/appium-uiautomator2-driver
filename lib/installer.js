...
const IGNORE_SHA512 = process.env.npm_config_uiautomator2_driver_server_ignore_sha || process.env.UIAUTOMATOR2_DRIVER_SERVER_IGNORE_SHA || false; // only for dev purposes
...
async function downloadUiAutomator2ServerApk () {
  ...
  if ( serverFingerprint !== SERVER_DOWNLOAD_SHA512 && !IGNORE_SHA512 ) {
    log.errorAndThrow(`bad Server SHA512 fingerprint: ${serverFingerprint}`);
    log.error("Stopping the installation" );
    return;
  }
  if ( serverTestFingerprint !== SERVER_TEST_DOWNLOAD_SHA512 && !IGNORE_SHA512){
